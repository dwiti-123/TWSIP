...
public class Atm_process {
    public static void main(String[] args) {
        try {
            ...
            int spin=scp.nextInt();

                String sql="SELECT * FROM account WHERE account_no='"+acc_no+"'"+ "AND pin_no='"+spin+"'";
                ...
                if(rs.next()){
                    ...
                    if(select==1){
                        ...
                        String sqlbal="SELECT balance FROM account where account_no="+acc_no;
                        Statement stmtbal= conn.createStatement();
                        ResultSet b = stmtbal.executeQuery(sqlbal);
                ...
                //if user does not have account
                    ...
                    System.out.println("\n 1 for  Reenter Password And Account \n 2 for Reset Password \n 2 for Creating new Account");
                    Scanner se1=new Scanner(System.in);
                    int select1= se1.nextInt();

                    if(select1 == 1)
                    {
                        System.out.println("Reload System......");
                    }
                   ...
                   if(select1==2)
                    {
                        ...
                        PreparedStatement ps1= conn.prepareStatement("UPDATE account SET pin_no=? WHERE account_no=?");
                        ps1.setInt(1,npin);
                        ps1.setInt(2,acc_no);
                        int a = ps1.executeUpdate();
